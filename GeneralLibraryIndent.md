...
<br> If 2 coordinates are provided in the table, the third is assumed to be 0.
<br>Valid Arguments:
<code>
...
Notes: This doesn't compute the distance using the typical "Euclidean distance" that you might be familiar with, but rather the "1-Norm," so that the result is the number of tiles an air unit would have to cross to get from A to B.  The distance between adjacent tiles on the same map is always 2 in the "1-Norm," even diagonal movement.  This function divides the coordinate distance by 2 to get the result in terms of tiles.
</p>
</details>
...
<details>
  ...
  gen.wondermodified -->
  ```
  Returns the movement allowance of a unit, after taking into account nuclear power and wonders.  Returns ["atomic" movement points](Jargon.md#atomic-movement-points).  
  Valid Arguments:
  ```
  unit: unitType
  ```
